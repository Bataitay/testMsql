...
class NHANVIEN extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i = 0; $i < $limit; $i++) {
            $data = [
                'HO'            => $this->randomFirstName(),
                'TEN'           => $this->randomLastName(),
                'NGAYSINH'      => $this->randomBirthday(),
                'NGAYLAMVIEC'   => $this->randomHireDay(),
                'DIACHI'        => $this->randomAdress(),
                'DIENTHOAI'     => $this->randomPhone(),
                'LUONGCOBAN'    => $this->randomSalary(),
                'PHUCAP'        => $this->randomBonus(),
            ];
    ...
    private function randomBirthday(){
        $min        = strtotime("40 years ago");
        $max        = strtotime("50 years ago");
        $rand_time  = rand($min, $max);
        $birth_date = date('Y-m-d', $rand_time);
        return $birth_date;
    }
    private function randomHireDay(){
        $min        = strtotime("40 years ago");
        $max        = strtotime("50 years ago");
        $rand_time  = rand($min, $max);
        $birth_date = date('Y-m-d', $rand_time);
    ...
    private function randomSalary(){
        return rand(7000000,15000000);
    }
    private function randomBonus(){
        return rand(400000,1000000);
    }
}
